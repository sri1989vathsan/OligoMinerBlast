def finalprobe(readoutprobedoc,primerdoc,nprobes,folder,transcriptlist,primernum):
    ...
    for fil in range(0,len(files)):
        ...
        blastres = pd.read_csv("."+files[fil].split(".")[1]+".txt",sep="\t",header=None)
        ...
        blasttrnares3 = blasttrnares2[np.argwhere(blasttrnares2[:,11]>40).flatten(),:]

        
        blastrrnares = pd.read_csv("."+files[fil].split(".")[1]+"rRNA.txt",sep="\t",header=None)
        blastrrnares2 = blastrrnares.to_numpy()
        blastrrnares3 = blastrrnares2[np.argwhere(blastrrnares2[:,11]>40).flatten(),:]
        ...
        if(len(fina)>nprobes):
            ...
            randlist2 = list(range(0,nprobes))
        ...
        for t in range(0,len(final)):
            ...
            rev.append([gene+"_RO_"+temp2+"_"+finprobes[t].split("#")[1], temp])
            revcom.append([gene+"_RO_"+temp2+"_"+finprobes[t].split("#")[1], primelist2[primernum,1]+readlist2[fil,1]+temp+readlist2[fil,1]+primelist2[primernum,2]])
        ...
        lists.append([gene,str(num),lengen])

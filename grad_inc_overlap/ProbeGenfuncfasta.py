    ...
    class SequenceCrawler:
        ...
        def run(self):
            ...
            while i < int(blockLen) - int(self.l):
                ...
                if self.seqCheck(self.block[i:i + self.l], i):
                    ...
                    if not (i + j + self.l >= int(blockLen) or j >= sizeRange):
                        ...
                        cands.append((str(startPos), str(startPos + j + self.l - 1),
                                    str(self.block[i:i + j + self.l]), str(0), str(repeatval)))
                        if self.verbocity:
                ...
                #print(i)
                #print(ncheckval)
            print("Before: "+str(len(cands)))
            
            initprobes = len(cands)
            self.sp2=33
            
            while (len(cands)<50 and self.sp2>10):
                self.sp2 -= 1
                #print("Overlap: "+str(30-self.sp2)+"; #Probes:"+ str(len(cands)))
                ...
                while i < int(blockLen) - int(self.l):
                    ...
                    if self.seqCheck(self.block[i:i + self.l], i):
                        ...
                        if not (i + j + self.l >= int(blockLen) or j >= sizeRange):
                            startPos = self.start + i
                            cands.append((str(startPos), str(startPos + j + self.l - 1),
                                        str(self.block[i:i + j + self.l]),str(initprobes),str(repeatval)))
                            if self.verbocity:
                                print ('Picking a candidate probe of %d bases starting '
                                    'at base %d' % (self.l + j, startPos))
                            if self.reportVal:
            ...
            print("After: "+str(len(cands))+" Overlap: "+str(30-self.sp2))
            
            # Determine the stem of the input filename.
            fileName2 = str(self.inputFile).split('.')[2]
            ...
            if self.bedVal:
                ...
                for i, (start, end, seq, inprobes, ovlp) in enumerate(cands):
                ...
                # Build the output file.
                for i, (start, end, seq, inprobes, ovlp) in enumerate(cands):
                    nameprobe = ">Probe#"+str(i+1)+"_"+str(inprobes)+"_"+str(ovlp)
                    outList.append('%s\n%s'%(nameprobe,seq))
